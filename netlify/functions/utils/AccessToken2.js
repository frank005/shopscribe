// Browser and Node.js compatible AccessToken2.js
...
if (isNode) {
    try {
        ...
        // TextEncoder/TextDecoder are available in Node.js 18+, but ensure they exist
...
// HMAC function - works in both browser and Node.js
async function encodeHMac(key, message) {
    ...
    if (nodeCrypto) {
        const hmac = nodeCrypto.createHmac('sha256', Buffer.from(keyData))
        hmac.update(Buffer.from(messageData))
...
class AccessToken2 {
    ...
    async build() {
        ...
        if (!pako) {
            throw new Error('pako library is not loaded. Install with: npm install pako')
        }
